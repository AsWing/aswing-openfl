 ...
 */
    ...
    override public function setParent(parent:MutableTreeNode):Void {
        ...
        if (parent != null) {
            path = (cast(parent, FHTreeStateNode)).getTreePath().pathByAddingChild(getUserObject());
 ...
 */
    ...
    override public function removeAt(childIndex:Int):Void {
        var node:FHTreeStateNode = cast(getChildAt(childIndex), FHTreeStateNode);
 ...
 */
    ...
    override public function setUserObject(o:TreeNode):Void {
        ...
        if (path != null) {
            var parent:FHTreeStateNode = cast(getParent(), FHTreeStateNode);
...
// PENDING: Make this a binary search!
        for (counter in 0...getChildCount()) {
            if ((cast(getChildAt(counter), FHTreeStateNode)).childIndex == index)
                return cast(getChildAt(counter), FHTreeStateNode);
        }
        return null;
    }
...
// for(; counter < maxCounter; counter++) {
        for (counter in 0...maxCounter) {
            child = cast( getChildAt(counter), FHTreeStateNode);
...
// child count directly from that!
                var nextSibling:FHTreeStateNode = cast(parent.getChildAt(pIndex + 1), FHTreeStateNode);
                ...
                for (counter in 0...getChildCount()) {
                    retCount += (cast(getChildAt(counter), FHTreeStateNode)).getTotalChildCount();
                }
                return retCount;
            }
        }
 ...
 */
    ...
    private function resetChildrenPaths(parentPath:TreePath):Void {
        ...
        layoutCache.addMapping(this);
        for (counter in 0...getChildCount()) {
            (cast(getChildAt(counter), FHTreeStateNode)).resetChildrenPaths(path);
        }
    }
 ...
 */
    ...
    private function removeFromMapping():Void {
        if (path != null) {
            layoutCache.removeMapping(this);
            for (counter in 0...getChildCount()) {
                (cast(getChildAt(counter), FHTreeStateNode)).removeFromMapping();
            }
        }
    }
 ...
 */
    ...
    public function createChildFor(userObject:Dynamic):FHTreeStateNode {
        ...
        var maxCounter:Int = getChildCount();
        for (counter in 0...maxCounter) {
            aNode = cast( getChildAt(counter), FHTreeStateNode);
 ...
 */
    ...
    public function adjustRowBy(amount:Int, startIndex:Int = -2):Void {
        ...
        if (startIndex == -2) {
            ...
            if (expanded) {
                for (counter in 0...getChildCount()) {
                    (cast(getChildAt(counter), FHTreeStateNode)).adjustRowBy(amount);
                }
            }
        } else {
...
// children following startIndex.
                for (counter in startIndex...getChildCount())(cast(getChildAt(counter), FHTreeStateNode)).adjustRowBy(amount);
            }
// Parent
            var parent:FHTreeStateNode = cast(getParent(), FHTreeStateNode);
...
// the row of a particular node fast is very nice!
    ...
    public function resetChildrenRowsFrom(newRow:Int, childIndex:Int,
        ...
        for (counter in childIndex...maxCounter) {
            node = cast(getChildAt(counter), FHTreeStateNode);
        ...
        node = cast(getParent(), FHTreeStateNode);
        if (node != null) {
...
// Adjust the rows.
            ...
            while (counter < maxCounter) {
                ...
                aChild = cast( getChildAt(counter), FHTreeStateNode);
...
// above.
                    while (counter < maxCounter) {
                        (cast( getChildAt(counter), FHTreeStateNode)).childIndex--;
                        counter++;
                    }
                    childCount--;
                    return;
...
//    for(var counter:int = 0; counter < maxCounter; counter++) {
        for (counter in 0...maxCounter) {
            (cast(getChildAt(counter), FHTreeStateNode)).childIndex += amount;
        }
    }
 ...
 */
    ...
    private function getCountTo(stopIndex:Int):Int {
        ...
        if (parent != null) {
            return retCount + (cast( getParent(), FHTreeStateNode)).getCountTo(childIndex);
        }
        if (!layoutCache.isRootVisible()) {
            return (retCount - 1);
        }
